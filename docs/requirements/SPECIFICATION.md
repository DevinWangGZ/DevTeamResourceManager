...
> **文档版本**：v1.6 | **更新日期**：2026-02 | **负责人**：项目组
...
#### 2.2.2 任务配合人
- **功能描述**：任务认领人可添加多名配合人，协同完成任务
- **适用场景**：一个任务需要多人协作时，由认领人协调分配人天
- **关键规则**：
  - 只有任务认领人可以管理配合人
  - 可操作的任务状态：已认领、进行中
  - 配合人必须是开发人员角色
  - 认领人不能将自己添加为配合人
  - 同一用户在同一任务中只能作为一次配合人
  - **人天约束**：所有配合人的分配人天之和 ≤ 任务拟投入人天，超出则不合理，系统拒绝
- **任务确认后的数据流**：
  - 主认领人的实际投入人天 → 汇入其工作量统计
  - 每位配合人的分配人天 → 分别汇入各自工作量统计
  - 配合人的任务记录 → 出现在其业务履历中
- **验收标准**：
  - 任务详情页展示配合人列表（姓名、分配人天）
  - 认领人可添加、调整人天、移除配合人
  - 添加时实时显示剩余可分配人天
  - 超出任务人天时系统提示错误并拒绝
...
#### 2.5 任务状态流转
- **主流程状态流**：`草稿 ⇄ 已发布（可退回） → 已认领 → 进行中 → 已提交 → 已确认 → 已归档`
- **派发流程状态流**：`草稿 → 已发布 → 待评估 → 已认领（接受）/ 已发布（拒绝）`
- **退回草稿**：已发布 → 草稿（创建者或PM可操作，退回后可重新编辑和发布）
- **验收标准**：
  - 状态流转有明确的规则和校验
  - 不支持非法状态流转
  - 状态变更记录可追溯
...
#### 11.1 Markdown编辑器
- **功能描述**：提供Markdown编辑器供用户创建和编辑知识文章
- **编辑器功能**：
  - 支持Markdown语法（标题、列表、代码块、表格、链接、引用等）
  - 实时预览（编辑和预览双栏显示）
  - 语法高亮
  - 自动保存草稿
  - 支持快捷键操作
...
- **功能描述**：支持在编辑器中粘贴和上传图片
- **功能要求**：
  - 支持从剪贴板粘贴图片（Ctrl+V / Cmd+V）
  - 支持拖拽上传图片
  - 支持点击上传图片文件
  - 图片自动上传到服务器
  - 图片自动转换为Markdown格式（`![alt](url)`）
  - 支持图片预览和删除
- **验收标准**：
  - 粘贴图片操作流畅，无需额外步骤
  - 图片上传速度快，有上传进度提示
...
#### 11.6 大模型集成准备
- **功能描述**：Markdown格式便于后续集成大模型优化文字描述
- **设计考虑**：
  - 文章内容以Markdown格式存储
  - 支持大模型API调用（预留接口）
  - 可以选中文本进行优化建议
...
### P2（增强功能，大部分已实现）
1. ✅ 团队能力洞察（技能矩阵、人才梯队分析）
2. ✅ 数据导出功能（Excel导出）
3. ✅ 消息通知系统
4. ✅ 知识分享（Markdown编辑器、图片上传、文章管理）
5. ❌ 智能候选人推荐（高级技能匹配算法，待实现）
...
| 版本 | 日期 | 更新内容 | 负责人 |
|------|------|----------|--------|
| v1.6 | 2026-02 | 新增任务配合人功能需求（2.2.2节） | 项目组 |
| v1.5 | 2026-02 | 新增任务退回草稿、任务编辑功能需求；更新需求优先级为实际完成状态 | 项目组 |
| v1.4 | 2024-05 | 拆分项目经理和开发组长角色，明确各自职责和权限 | 项目组 |
| v1.3 | 2024-05 | 新增知识分享功能需求（Markdown编辑器、图片上传、文章管理） | 项目组 |
